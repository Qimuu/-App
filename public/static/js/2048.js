...
function start() {
    var maxG = window.localStorage.getItem("2048MaxGrade");
    if(maxG == null) {
        maxG = 0;
    }
    document.getElementById("maxG").innerText = maxG;
}

function new_game() {
    var lastG = document.getElementById("score").innerText;
    var maxG = window.localStorage.getItem("2048MaxGrade");
    if(maxG == null) {
        maxG = 0;
    }else {
        maxG = Math.max(maxG, lastG);
    }maxG = Math.max(maxG, lastG);
    window.localStorage.setItem("2048MaxGrade", maxG);
    document.getElementById("maxG").innerText = maxG;
    for(var i = 1;i < 5;i++) {
        for(var j = 1;j < 5;j++) {
